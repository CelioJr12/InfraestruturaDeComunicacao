...
def parse_packet(packet: str):
    ...
    try:
        ...
        print(f"Erro de conversão em pacote: {packet!r}")
        return None
...
def main():
    ...
    # recebe handshake do cliente, formato: "<MODO>;<MAX_TAMANHO>\n" ou sem \n
    try:
    ...
    # aguarda chave publica dh do cliente ("DH|<client_pub>\n")
    ...
    while True:
        ...
        while '\n' in buffer:
            ...
            # calcula o checksum local do payload (checksum sobre a string base64)
            ...
            if modo == 'GBN':
                if seq_global != expected_seq:
                    # reenvia o ack cumulativo para sinalizar o que falta
                ...
                if seq_global < win_start:
                    # pacote já fora da janela (antigo): re-ack para suprimir retransmissão
